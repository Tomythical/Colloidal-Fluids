...
af = [0.69,0.70,0.71,0.72]
gr0 = []
gr1 = []
gr2 = []
gr3 = []
for q in range(len(af)):
    N = 100
    radius = 0.5
    diameter = radius*2
    nparticles = []
    ...
    dr = 0.05
    afL = math.sqrt(N*math.pi*(diameter**2)/(4*af[q]))
    ...
    def calc_gr():
        if q == 0:
            for i in range (1,len(Nrk)):
                gr0.append((Nrk[i]/50000)/(2*math.pi*i*dr*dr*avg_no_density))
        if q == 1:
            for i in range (1,len(Nrk)):
                gr1.append((Nrk[i]/50000)/(2*math.pi*i*dr*dr*avg_no_density))
        if q == 2:
            for i in range (1,len(Nrk)):
                gr2.append((Nrk[i]/50000)/(2*math.pi*i*dr*dr*avg_no_density))
        if q == 3:
            for i in range (1,len(Nrk)):
                gr3.append((Nrk[i]/50000)/(2*math.pi*i*dr*dr*avg_no_density))
...
np.savetxt("2d plot-{0.69}.dat", [rx,gr0])
np.savetxt("2d plot-{0.70}.dat", [rx,gr1])
np.savetxt("2d plot-{0.71}.dat", [rx,gr2])
np.savetxt("2d plot-{0.72}.dat", [rx,gr3])
# plt.plot(rx,gr0)
# plt.plot(rx,gr1)
# plt.plot(rx,gr2)
# plt.plot(rx,gr3)
...
# plt.xlabel('Radius (r)')
# plt.ylabel('Pair Distribution Function g(r)')
# plt.legend(["Area Fraction = 0.69", "Area Fraction = 0.70", "Area Fraction = 0.71", "Area Fraction = 0.72"], loc='upper right')
# plt.savefig('2D Mixed Plot.jpeg', dpi =250)
# plt.show()
