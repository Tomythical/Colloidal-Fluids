    ...
    for i in nparticles:
        if i is not org_particle:
            if overlaps(temp_particle, i) is True:
                x = True
                break
            if i.position[0] < diameter or i.position[0] > L - diameter or i.position[1] < diameter or i.position[1] > L - diameter or i.position[2] < diameter or i.position[2] > L - diameter:
                """ X-Y Plane """
                ...
                """ X-Z Plane """
                if temp_particle.position[0] < diameter and temp_particle.position[2] > diameter and temp_particle.position[2] < L -  diameter: #for x<0.5
                    temp_particle.position[0] += L
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[0] -= L
                if temp_particle.position[0] > L - diameter and temp_particle.position[2] > diameter and temp_particle.position[2] < L -  diameter: #for x>L-0.5
                    temp_particle.position[0] -= L
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[0] += L
                if temp_particle.position[2] < diameter and temp_particle.position[0] > diameter and temp_particle.position[0] < L -  diameter: #for y < 0.5
                    temp_particle.position[2] += L
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[2] -= L
                if temp_particle.position[2] > L - diameter and temp_particle.position[0] > diameter and temp_particle.position[0] < L -  diameter: #for y> L- 0.5
                    temp_particle.position[2] -= L
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                ...
                if temp_particle.position[0] < diameter and temp_particle.position[2] < diameter: #for x<0.5 and y < 0.5 i.e Bottom-left
                    temp_particle.position[0] += L                                            # Check overlap with top-right
                    temp_particle.position[2] += L
                    if overlaps(temp_particle, i) is True:
                        x = True
                    ...
                    temp_particle.position[2] += L                                            # Check overlap with top-left
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[2] -= L
                ...
                if temp_particle.position[0] < diameter and temp_particle.position[2] > L - diameter: #for x<0.5 and y > L -0.5 i.e Top-left
                    temp_particle.position[0] += L                                                # Check overlap with bottom-right
                    temp_particle.position[2] -= L
                    if overlaps(temp_particle, i) is True:
                        x = True
                    ...
                    temp_particle.position[2] -= L                                            # Check overlap with bottom-left
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[2] += L

                if temp_particle.position[0] > L - diameter and temp_particle.position[2] > L - diameter: #for x>L-0.5 and y > L - 0.5 i.e. Top-Right
                    temp_particle.position[0] -= L                                                     # Check overlap with bottom-left
                    temp_particle.position[2] -= L
                    if overlaps(temp_particle, i) is True:
                        x = True
                    ...
                    temp_particle.position[2] -= L                                            # Check overlap with bottom-right
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[2] += L

                if temp_particle.position[0] > L - diameter and temp_particle.position[2] < diameter: #for x> L - 0.5 and y < 0.5 i.e. Bottom-right
                    temp_particle.position[0] -= L                                                  # Check overlap with top-left
                    temp_particle.position[2] += L
                    if overlaps(temp_particle, i) is True:
                        x = True
                    ...
                    temp_particle.position[2] += L                                            # Check overlap with top-right
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[2] -= L
                ...
                """ Y-Z Plane """
                if temp_particle.position[1] < diameter and temp_particle.position[2] > diameter and temp_particle.position[2] < L -  diameter: #for x<1.5
                    temp_particle.position[1] += L
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[1] -= L
                if temp_particle.position[1] > L - diameter and temp_particle.position[2] > diameter and temp_particle.position[2] < L -  diameter: #for x>L-1.5
                    temp_particle.position[1] -= L
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[1] += L
                if temp_particle.position[2] < diameter and temp_particle.position[1] > diameter and temp_particle.position[1] < L -  diameter: #for y < 1.5
                    temp_particle.position[2] += L
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                    temp_particle.position[2] -= L
                if temp_particle.position[2] > L - diameter and temp_particle.position[1] > diameter and temp_particle.position[1] < L -  diameter: #for y> L- 1.5
                    temp_particle.position[2] -= L
                    if overlaps(temp_particle, i) is True:
                        x = True
                        break
                ...
                if temp_particle.position[1] < diameter and temp_particle.position[2] < diameter: #for x<1.5 and y < 1.5 i.e Bottom-left
                    temp_particle.position[1] += L                                            # Check overlap with top-right
                    temp_particle.position[2] += L
                    if overlaps(temp_particle, i) is True:
                        x = True
                ...
                if temp_particle.position[1] < diameter and temp_particle.position[2] > L - diameter: #for x<1.5 and y > L -1.5 i.e Top-left
                    temp_particle.position[1] += L                                                # Check overlap with bottom-right
                    temp_particle.position[2] -= L
                    if overlaps(temp_particle, i) is True:
                        x = True
                ...
                if temp_particle.position[1] > L - diameter and temp_particle.position[2] > L - diameter: #for x>L-1.5 and y > L - 1.5 i.e. Top-Right
                    temp_particle.position[1] -= L                                                     # Check overlap with bottom-left
                    temp_particle.position[2] -= L
                    if overlaps(temp_particle, i) is True:
                        x = True
                ...
                if temp_particle.position[1] > L - diameter and temp_particle.position[2] < diameter: #for x> L - 1.5 and y < 1.5 i.e. Bottom-right
                    temp_particle.position[1] -= L                                                  # Check overlap with top-left
                    temp_particle.position[2] += L
                    if overlaps(temp_particle, i) is True:
                        x = True
...
def calc_gr():
    ...
    for i in range (1,len(Nrk)):
        gr.append((Nrk[i]/(N*dr*350*20))/(4*math.pi*((i*dr)**2)*dr*avg_no_density))
    return gr
...
def graph2(x,y):
    ...
    plt.title(f"Volume Fraction - {round(area_fraction,3)}, L - {L}, N - {N}")
    plt.show()
...
if __name__ == '__main__':
    ...
    graph2(rx,calc_gr())
    # np.savetxt(f"3d plot-{N}.dat", [rx,calc_gr()])
